...
import player from "@normalized:Y&&&libplayer.so&";
const DOMAIN = 0x0000;
...
export class VideoPlayViewModel {
    ...
    @Track
    chooseSpeed: number = Const.VIDEO_DEFAULT_SPEED;
    @Track
    isUse: boolean = false;
    @Track
    isSpeedListVisible: boolean = false;
    @Track
    ...
    @Track
    speedText: string | Resource = { "id": 16777220, "type": 10003, params: [], "bundleName": "com.example.decodeplaycontrol", "moduleName": "entry" };
    @Track
    isSeek: boolean = false;
    @Track
    playState: PlayerState = PlayerState.IDLE;
    ...
    async onVideoSelected(index: number, rawDes: resourceManager.RawFileDescriptor): Promise<PlayerState | null> {
        ...
        this.speedText = { "id": 16777220, "type": 10003, params: [], "bundleName": "com.example.decodeplaycontrol", "moduleName": "entry" };
        if (!rawDes) {
